// Originally taken from shadertoy (https://www.shadertoy.com/view/Wt33Wf)
// All credits to the original author kaiware007
// (https://www.shadertoy.com/user/kaiware007)
...
float sdCloud(in vec2 p, in vec2 a1, in vec2 b1, in vec2 a2, in vec2 b2,
              float w) {
...
void main() {
  vec2 uv = (2.0 * vec2(gl_FragCoord.x, iResolution.y - gl_FragCoord.y) -
             iResolution.xy) /
            iResolution.y;
  float battery = 1.0;
  ...
  // if (abs(uv.x) < (9.0 / 16.0))
  {
    ...
    if (uv.y < -0.2) {
      ...
      // col = mix( col,
      // vec3(1.0, 1.0, 1.0), 1.0-smoothstep(0.03,0.04,abs(fujiVal)) ); col =
      // vec3(1.0, 1.0, 1.0) *(1.0-smoothstep(0.03,0.04,abs(fujiVal)));

      // horizon color
      col += mix(col,
                 mix(vec3(1.0, 0.12, 0.8), vec3(0.0, 0.0, 0.2),
      ...
      float cloudVal1 = sdCloud(
          cloudUV, vec2(0.1 + sin(cloudTime + 140.5) * 0.1, cloudY),
      ...
      float cloudVal2 = sdCloud(
          cloudUV, vec2(-0.9 + cos(cloudTime * 1.02 + 541.75) * 0.1, cloudY),
      ...
      col += vec3(1.0, 1.0, 1.0) *
             (1.0 - smoothstep(0.0, 0.01, abs(cloudVal - 0.075)));
    }
